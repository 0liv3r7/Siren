...
fn link(_: Request, cx: RouteContext<Config>) -> Result<Response> {
    ...
    let vless_link = format!("vless://{uuid}@{host}:443?encryption=none&type=ws&host={host}&path=%2FFree%2FJP&security=tls&sni={host}#siren vless");
    let trojan_link = format!("trojan://{uuid}@{host}:443?encryption=none&type=ws&host={host}&path=%2FFree%2FJP&security=tls&sni={host}#siren trojan");
    let ss_link = format!("ss://{}@{host}:443?plugin=v2ray-plugin%3Btls%3Bmux%3D0%3Bmode%3Dwebsocket%3Bpath%3D%2FFree%2FJP%3Bhost%3D{host}#siren ss", URL_SAFE.encode(format!("none:{uuid}")));
